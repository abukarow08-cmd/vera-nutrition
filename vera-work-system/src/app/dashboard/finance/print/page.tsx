  ...
  return (
    <div style={{ padding: '40px', fontFamily: 'Inter, sans-serif', maxWidth: '900px', margin: '0 auto', color: '#111' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '32px' }}>
        {[
          { label: 'Total Income', value: `${totalIncome.toLocaleString('en-US', {style:'currency',currency:'USD'})}`, color: '#16a34a' },
          { label: 'Total Expenses', value: `${totalExpenses.toLocaleString('en-US', {style:'currency',currency:'USD'})}`, color: '#dc2626' },
          { label: 'Net Profit', value: `${net.toLocaleString('en-US', {style:'currency',currency:'USD'})}`, color: net >= 0 ? '#2357A3' : '#dc2626' },
        ].map(k => (
          ...
          {records.map((r, i) => (
            <tr key={r.id} style={{ background: i % 2 === 0 ? 'white' : '#F8FAFB', borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '9px 12px', textAlign: 'right', fontWeight: 600, color: (r.type === 'income' || r.type === 'in') ? '#16a34a' : '#dc2626' }}>
                {(r.type === 'income' || r.type === 'in') ? '+' : '-'}{Number(r.amount).toLocaleString('en-US', {style:'currency',currency:'USD'})}
              </td>
            </tr>
          ))}
        </tbody>
